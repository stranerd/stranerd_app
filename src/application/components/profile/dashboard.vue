<template>
	...
	<template v-else>
		<div v-if="user" class="col-span-12 flex flex-col px-3">
			<div
				...
				<div class="flex flex-col justify-center items-center w-full lg:w-1/2 ">
					<div class="py-3">
						<progress-bar :base-color="'#CAE2E8'" :dimension="100" :progress="user.score" :radius="90"
							:stroke="15" :stroke-color="'#546DD3'"></progress-bar>
					</div>
				</div>
			</div>
			<div class="flex flex-row flex-wrap mt-4 mb-7">
				<div class="w-1/2 pr-2">
					<div class="flex flex-col rounded-lg bg-light_gray py-9 px-4 gap-3 justify-center items-center">
						...
						<h4 class="text-base md:text-lg text-dark_gray font-bold">{{ formatNumber(user.meta.questions)
						}}</h4>
					</div>
				</div>
				<div class="w-1/2 pl-2">
					<div class="flex flex-col rounded-lg bg-light_gray py-5 px-4 gap-3 justify-center items-center">
						...
						<h4 class="text-base md:text-lg text-dark_gray font-bold">{{ formatNumber(user.meta.answers)
						}}</h4>
				...
				<div class="w-1/2 pr-2 mt-2 lg:mt-4">
					<div class="flex flex-col rounded-lg bg-light_gray py-9 px-4 gap-3 justify-center items-center">
						...
						<h4 class="text-base md:text-lg text-dark_gray font-bold">{{ formatNumber(user.meta.sessions)
						}}</h4>
					</div>
				</div>
				<div class="w-1/2 pl-2 mt-2 lg:mt-4">
					<div class="flex flex-col rounded-lg bg-light_gray py-9 px-4 gap-3 justify-center items-center">
						...
						<h4 class="text-sm md:text-lg text-dark_gray font-bold">{{ formatTime(user.dates.createdAt)
						}}</h4>
					</div>
				</div>
...
const progressBar = defineAsyncComponent(() => import('@app/components/profile/progressRing.vue'))
