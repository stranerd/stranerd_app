	...
	<div class="w-full flex flex-col text-xs mg:text-sm">
		...
		<div class="mt-3 border border-faded_gray rounded-lg py-5 flex-col px-3">
			...
			<ion-textarea rows="9" v-model="factory.body"
				...
				placeholder="Write out the detailed explanation of the answer you gave above. (Optional)"></ion-textarea>
		</div>
		...
		<div class="flex flex-row px-3 my-7 md:gap-5 text-white">
			<div class="w-1/2 flex flex-row justify-center items-center">
				<button class=" md:w-full px-6 py-3 relative ion-activatable rounded-lg  font-bold bg-dark_gray " @click="showAddAnswer = false">
					Cancel
					<ion-ripple-effect class="rounded-lg"></ion-ripple-effect>
				</button>
			</div>
			<div class="w-1/2 flex flex-row justify-center items-center">
				<button class="md:w-full px-6 relative ion-activatable font-bold py-3 rounded-lg bg-primary" @click="createAnswer">
					Post answer
					<ion-ripple-effect class="rounded-lg"></ion-ripple-effect>
				</button>
			</div>
...
import { showAddAnswer, useCreateAnswer } from '@/application/composable/questions/answers'
...
export default defineComponent({
	...
	setup(props) {
		...
		const { factory, error, loading, answeringQuestion, createAnswer} = useCreateAnswer(props.question ? props.question : null)
