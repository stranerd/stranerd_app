<template>
	<div class="w-full  h-auto flex bg-primaryBg py-12 ">
		<div
			class="container mx-auto text-bodyText p-6 flex flex-col-reverse lg:flex-row gap-6 lg:gap-16 md:items-center justify-between">
			<div class="flex flex-col md:gap-5 gap-3 lg:max-w-[47%] text-primaryText">
				<h2 class="md:text-4xl text-2xl font-bold">Download our App</h2>
				<p class="md:text-lg text-sm">
					Wherever you may be, carry Stranerd in your pocket. Get full access from anywhere at anytime with your mobile phone.
				</p>
				<div class="flex gap-4 items-center">
					<a href="https://play.google.com/store/apps/details?id=com.stranerd.app" rel="noopener noreferrer"
						target="_blank" class="overflow-hidden">
						<img alt="playstore button" class="md:h-16" src="@/assets/images/newLanding/play_store_white.png" />
					</a>
					<a href="https://play.google.com/store/apps/details?id=com.stranerd.app" rel="noopener noreferrer"
						target="_blank" class="overflow-hidden"> 
						<img alt="playstore button" class="md:h-16" src="@/assets/images/newLanding/apple_store_white.png" />
					</a>
				</div>
			</div>
			<div class="w-auto lg:w-1/2 relative">
				<img alt="hero" class=" rounded-2xl" src="@/assets/images/newLanding/mobile_phone.png" />
				<img alt="app_preview" src="@/assets/images/newLanding/app_preview.png" class="w-[46%] absolute top-[19%] left-[35%]">
			</div>
			
		</div>
	</div>
</template>

<style scoped>

a {
	@apply rounded-lg h-12 md:h-16 w-[116px] md:w-[165px] md:text-lg text-xs
}
</style>