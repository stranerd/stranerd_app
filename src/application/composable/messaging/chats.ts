...
export const useClassLibrary = (classId: string) => {
	if (classGlobal[classId] === undefined) {
		const listener = useListener(async () => {
			return ChatsUseCases.listenToClassLibrary(classId, {
				created: async (entity) => {
					addToArray(classGlobal[classId].chats.value, entity, (e) => e.id, (e) => e.createdAt)
				},
				updated: async (entity) => {
					addToArray(classGlobal[classId].chats.value, entity, (e) => e.id, (e) => e.createdAt)
				},
				deleted: async (entity) => {
					const index = classGlobal[classId].chats.value.findIndex((c) => c.id === entity.id)
					if (index !== -1) classGlobal[classId].chats.value.splice(index, 1)
				}
			})
		})
		classGlobal[classId] = {
	...
	const fetchChats = async () => {
		...
		try {
			...
			const chats = await ChatsUseCases.getClassLibrary(classId)
			chats.results.map((d) => addToArray(classGlobal[classId].chats.value, d, (e) => e.id, (e) => e.createdAt))
	...
	onMounted(async () => {
		if (!classGlobal[classId].fetched.value && !classGlobal[classId].loading.value) await fetchChats()
