...
export class FacultyRepository implements IFacultyRepository {
	private dataSource: FacultyBaseDataSource
	private transformer: FacultyTransformer

	constructor (dataSource: FacultyBaseDataSource, transformer: FacultyTransformer) {
	...
	async listenToOne (id: string, listener: Listeners<FacultyEntity>) {
	...
	async listenToMany (query: QueryParams, listener: Listeners<FacultyEntity>, matches: (entity: FacultyEntity) => boolean) {
