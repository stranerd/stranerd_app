...
export class DiscussionTransformer {
	fromJSON (model: DiscussionFromModel) {
		const { id, content, media, links, groupId, classId, user, readAt, createdAt, updatedAt } = model
		return new DiscussionEntity({
			id, content, media, links, groupId, classId, user, readAt, createdAt, updatedAt
		})
	}
