<template>
	<NuxtLink :to="`/sessions/${meta.id}`" class="gap-0-5">
		...
		<div class="flex-grow-1 text-truncate">
			<div class="flex justify-content-between items-center gap-0-5">
				<DynamicText :truncate="true" class="name">
					{{ meta.fullName }}
				</DynamicText>
				<DynamicText>{{ formatTime(meta.last.createdAt, true) }}</DynamicText>
			</div>
			<div class="flex items-center text-truncate gap-0-25">
				...
				<span
					...
					class="bg-success ms-auto flex items-center justify-center rounded-pill text-white"
